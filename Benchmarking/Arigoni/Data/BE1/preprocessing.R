...
load.data = function () {
  ...
  for (h in 1:H) {
    ...
    folder = cell.list[h]
    barcodes = paste("SGD_Data/BE1/raw", folder, "barcodes.tsv.gz", sep = "/")
    features = paste("SGD_Data/BE1/raw", folder, "features.tsv.gz", sep = "/")
    metrics = paste("SGD_Data/BE1/raw", folder, "metrics_summary.csv", sep = "/")
    matrix = paste("SGD_Data/BE1/raw", folder, "matrix.mtx.gz", sep = "/")
...
if (SAVE) {
  ...
  writeMM(mat, file = "SGD_Data/BE1/filtered/counts.mtx")
  write.table(bar, file = "SGD_Data/BE1/filtered/barcodes.csv", sep = ",", dec = ".", row.names = FALSE, col.names = FALSE)
  write.table(cll, file = "SGD_Data/BE1/filtered/celltype.csv", sep = ",", dec = ".", row.names = FALSE, col.names = FALSE)
  write.table(ftr[,1:3], file = "SGD_Data/BE1/filtered/features.csv", sep = ",", dec = ".", row.names = FALSE, col.names = FALSE)
}

SAVERDS = TRUE
if(SAVERDS){
    saveRDS(mat, file = "SGD_Data/BE1/filtered/counts.RDS")
    saveRDS(bar, file = "SGD_Data/BE1/filtered/barcodes.RDS")
    saveRDS(cll, file = "SGD_Data/BE1/filtered/celltype.RDS")
    saveRDS(ftr[,1:3], file = "SGD_Data/BE1/filtered/features.RDS")
