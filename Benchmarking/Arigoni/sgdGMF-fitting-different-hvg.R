...
for(j in c(100,200,1000,1500,2000)){

  hvg <- (rownames(sim_full)[order(rowData(sim_full)$biological.var, decreasing = TRUE)])[1:j]
  sim <- sim_full[hvg,]
  ...
  saveRDS(object = gmf.fit.list, file = paste0("Output_models/sgdGMF-B-SGD-fit-list-", j, "-hvg.RDS"))
}
