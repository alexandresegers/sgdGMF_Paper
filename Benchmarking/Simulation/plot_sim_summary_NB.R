...
load.data = function (n, m, d, i) {
  ...
  filepath = FILEPATH
  filezip = "summary_sim_NB.zip"
  fileid = paste("_n", n, "_m", m, "_d", d, "_i", i, sep = "")
  filename = paste("summary_sim", fileid, "_NB.csv", sep = "")
...
filter.data = function (df) {
  ...
  old.models = c("AIRWLS-Pois", "Newton-Pois", "SGD-Pois", "AIRWLS-NB", "Newton-NB", "SGD-NB")
  new.models = c("AIRWLS-Pois", "Newton-Pois", "aSGD-Pois", "AIRWLS-NB", "Newton-NB", "aSGD-NB")
  ...
  # Set the algorithm variable
  df$algorithm = sapply(strsplit(as.character(df$model), "-"), \(.) .[1])
  df$algorithm = factor(df$algorithm, levels = c("AIRWLS", "Newton", "aSGD"))

  # Set the model family variable
  df$family = as.factor(sapply(strsplit(as.character(df$model), "-"), \(.) .[2]))
  df$family = factor(df$family, levels = c("NB", "Pois"))
  ...
  df = df %>% dplyr::select(iteration, model, family, algorithm, dimension, rank, time,
...
merge.data = function (df1, df2, melt = FALSE) {
  ...
  df = df %>%
    group_by(model, family, algorithm, setting, dimension, rank) %>%
    summarise(time = mean(time, na.rm = TRUE),
              memory = mean(memory, na.rm = TRUE),
              error = mean(100*error, na.rm = TRUE, trim = 0.04),
              deviance = mean(100*deviance, na.rm = TRUE),
              silhouette = mean(silhouette, na.rm = TRUE),
              purity = mean(purity, na.rm = TRUE),
              .groups = "drop") %>%
  ...
  if (melt) {
    ...
    df = reshape2::melt(df, id.vars = c("model", "family", "algorithm", "setting", "dimension"))
  }
...
df.flat.tmp = df.flat %>% mutate(error = 5*error*(dimension)^-.4) # 25)
df.deep.tmp = df.deep %>% mutate(error = 5*error*(dimension)^-.4) # 25)
...
plt = df.melt %>%
  ggplot(map = aes(x = dimension, y = value, color = algorithm, shape = family, linetype = family)) +
...
df.A.time_memory = rbind(df.A.time, df.A.memory) %>%
  mutate(variable = factor(variable, levels = c("Time", "Memory"),
                           labels = c("Execution time (s)", "Peak memory consumption (MB)")))
...
plt.A.time_memory = df.A.time_memory %>%
  ...
  scale_shape_manual(values = c(15, 17)) +
  scale_linetype_manual(values = c(2, 3)) +
  labs(x = "Matrix dimension", y = "Execution time (s)",
       color = "Method", shape = "Family", linetype = "Family",
...
plt.B.time_memory = df.B.time_memory %>%
  ...
  scale_shape_manual(values = c(15, 17)) +
  scale_linetype_manual(values = c(2, 3)) +
  labs(x = "Latent space rank", y = "Execution time (s)",
       color = "Method", shape = "Family", linetype = "Family",
...
if (SAVE) {
  filepath = IMGPATH
  filename = "simulation_time_memory_NB.pdf"
  zoom = 3.75
  nrows = 3
  ncols = 3
  width = 1 * ncols
...
plt.A.gof = df.A.gof %>%
  ggplot(map = aes(x = dimension, y = value, color = algorithm, shape = family, linetype = family)) +
  geom_line() + geom_point(size = 3) +
  facet_grid(rows = vars(variable), scale = "free", labeller = label_parsed) +
  scale_x_continuous(breaks = c(10, 25, 50, 75, 100)) +
  scale_shape_manual(values = c(15, 17)) +
  scale_linetype_manual(values = c(2, 3)) +
  labs(x = "Matrix dimension", y = "Goodness of fit measure",
       color = "Method", shape = "Family", linetype = "Family",
       title = TeX(paste("Setting A ($n \\uparrow, m \\uparrow, d = 5$)"))) +
  theme(legend.position = "right",
        legend.title = element_text(size = 13),
...
plt.B.gof = df.B.gof %>%
  ...
  scale_shape_manual(values = c(15, 17)) +
  scale_linetype_manual(values = c(2, 3)) +
  labs(x = "Latent space rank", y = "Goodness of fit measure",
       color = "Method", shape = "Family", linetype = "Family",
       title = TeX(paste("Setting B ($n = 5000, m = 500, d \\uparrow$)"))) +
  theme(legend.position = "right",
        legend.title = element_text(size = 13),
...
plt = ggpubr::ggarrange(plt.A.gof, plt.B.gof,
                        ...
                        legend = "bottom", align = "hv", labels = "AUTO")
...
if (SAVE) {
  filepath = IMGPATH
  filename = "simulation_gof_summary_NB.pdf"
  zoom = 3.75
  nrows = 5
  ncols = 3
  width = 1 * ncols
